    ...
    protected BuiltinStreamConsumer<WinningBid> setupWinningBidConsumer() {
        return winningBid -> {
            if (winningBid.getWorkerId().equals(getName())) {
                ...
                String repositoryName = winningBid.getJobName();
                if (repositoryName.contains("__")) {
                    jobNames.add(repositoryName.split("__")[0]);
                }
...
//            ackQueue.send(new BiddingFinishedAck(getName(), winningBid.getJobId()));
    ...
    private boolean biddingFinished(String jobId) {
        List<Bid> bids = bidMap.get(jobId);
        return bids.size() > 0 && bids.size() == getWorkerCount();
    }
...
//            if (
//                    workerIsAlive(workerId) &&
//                    workerNotTooBusy(workerId) &&
//                    !rejectedWorkerIds.contains(workerId)
//            ) {
//                return workerId;
//            }
//        }
...
//        resetAcks(false);
    ...
    private boolean workersTooBusy() {
        return workersWorkloads.values()
                .stream()
                .mapToInt(List::size)
...
//        EstimationMetrics estimationMetrics = new EstimationMetrics(job.jobId, getName(), estimatedTime, actualCost, LocalDateTime.now());
//        try {
...
//    }
    ...
    public boolean jobNameExists(String path) {
        String repositoryPath = path.split("__")[0];
        return jobNames.contains(repositoryPath) || jobNames.contains(path);
    }
}
