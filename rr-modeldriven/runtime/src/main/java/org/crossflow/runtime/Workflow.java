...
public abstract class Workflow<E extends Enum<E>> {
    ...
    public static final String CACHE_LOCATION = "experiment/techrank/in/";
    ;
    ...
    private BuiltinStream<CrossflowMetrics> metrics;
    ...
    private KVSRepository repository = new KVSRepository(redisHost, redisPort);
...
//											+ mbView.getInFlightCount());
    ...
    public String getPreferredWorkerIdFor(Job job) {
        ...
        Set<String> workerIds = repository.getWorkerIds(jobHash);
        if (workerIds.size() == 1) {
            return workerIds.iterator().next();
        }

        if (workerIds.size() > 1) {
            return repository.getLeastBusyWorkerOf(workerIds);
        }
        return null;
    }
...
//		return !max.getKey().equals(workerId);
    ...
    public synchronized void terminate() {
        ...
        System.out.println(getName() + ": " + totalWorkTime + " ms");
    ...
    public Set<String> getWorkerIdsForJob(Job job) {
        return repository.getWorkerIds(job.jobHash);
    }
    ...
    public void deleteCachedRepositories() {
        File cacheFolder = new File(CACHE_LOCATION);
