...
public abstract class Workflow<E extends Enum<E>> {
    ...
    private BuiltinStream<EstimationMetrics> estimationMetricsQueue;
    ...
    protected ConcurrentHashMap<String, Job> jobsWaiting = new ConcurrentHashMap<>();
    ...
    protected Map<String, WorkCost> estimatedWorkCosts = new HashMap<>();
    private Map<String, Boolean> acks = new HashMap<>();

    private String storageDir = "";
    private int workerCount = 5;
    private List<Bid> seenBids = new ArrayList<>();
    private List<WinningBid> seenWinningBids = new ArrayList<>();

    private BuiltinStream<LogMessage> masterLoggingTopic = null;
    protected Set<String> jobNames = new HashSet<>();
    private HashMap<String, List<String>> workersWorkloads = new HashMap<>();
    private BuiltinStream<ExtendedEstimationMetric> extendedEstimationMetricsQueue;
     ...
     */
    protected boolean enablePrefetch = true;
    ...
    public Workflow() {
        ...
        bidsQueue = new BuiltinStream<>(this, "BidsQueue", false);
        winningBids = new BuiltinStream<>(this, "WinningBids");
        ackQueue = new BuiltinStream<>(this, "BiddingFinishedAcks", false);
        metrics = new BuiltinStream<>(this, "Metrics", false);
        masterLoggingTopic = new BuiltinStream<>(this, "MasterLoggingTopic");
        estimationMetricsQueue = new BuiltinStream<>(this, "EstimationMetricsQueue", false);
        extendedEstimationMetricsQueue = new BuiltinStream<>(this, "ExtendedEstimationMetricsQueue", false);
    ...
    public void sendLogMessage(String message) {
        LogMessage logMessage = new LogMessage(this)
                ...
                .instanceId(getName());
        try {
            masterLoggingTopic.send(logMessage);
        } catch (Exception e) {
    ...
    protected void connect() throws Exception {
        ...
        ackQueue.init();
        metrics.init();
        masterLoggingTopic.init();
        estimationMetricsQueue.init();
        extendedEstimationMetricsQueue.init();
        ...
        if (isMaster()) {
            ...
            masterLoggingTopic.addConsumer(setupMasterLoggingConsumer());
            estimationMetricsQueue.addConsumer(setupEstimationMetricsConsumer());
            extendedEstimationMetricsQueue.addConsumer(setupExtendedEstimationMetricsConsumer());
        }
        ...
        if (isMaster()) {
            ...
            if (enableTaskMetadataTopic) {
                ...
                taskStatusTopic.addConsumer(new BuiltinStreamConsumer<>() {
            ...
            failedJobsTopic.addConsumer(failedJob -> {
...
//											+ mbView.getInFlightCount());
    ...
    protected List<EstimationMetrics> estimationMetrics = new ArrayList<>();
    protected List<ExtendedEstimationMetric> extendedEstimationMetrics = new ArrayList<>();

    private BuiltinStreamConsumer<EstimationMetrics> setupEstimationMetricsConsumer() {
        return estimationMetrics -> {
            this.estimationMetrics.add(estimationMetrics);
            String workerId = estimationMetrics.getWorkerId();
            List<String> jobNames = workersWorkloads.getOrDefault(workerId, new ArrayList<>());
            List<String> newJobNames = jobNames.stream()
                    .filter(jobId -> !jobId.equals(estimationMetrics.getJobId()))
                    .collect(Collectors.toList());
            workersWorkloads.put(workerId, newJobNames);
        };
    }

    private BuiltinStreamConsumer<ExtendedEstimationMetric> setupExtendedEstimationMetricsConsumer() {
        return metric -> {
            System.out.println(metric.toCSV());
            this.extendedEstimationMetrics.add(metric);
            String workerId = metric.getWorkerId();
            List<String> jobNames = workersWorkloads.get(workerId);
            List<String> newJobNames = jobNames.stream()
                    .filter(jobId -> !jobId.equals(metric.getJobId()))
                    .collect(Collectors.toList());
            workersWorkloads.put(workerId, newJobNames);
        };
    }

    private BuiltinStreamConsumer<LogMessage> setupMasterLoggingConsumer() {
        return logMessage -> {
            String formattedMessage = String.format("[%d] [%s] %s", logMessage.getTimestamp(), logMessage.getInstanceId(), logMessage.getMessage());
    ...
    protected BuiltinStreamConsumer<WinningBid> setupWinningBidConsumer() {
        return winningBid -> {
            if (winningBid.getWorkerId().equals(getName())) {
                estimatedBids.put(winningBid.getJobId(), winningBid.getWorkCost().getProcessingCost());
                estimatedWorkCosts.put(winningBid.getJobId(), winningBid.getWorkCost());
                jobNames.add(winningBid.getJobName());
            }
            if (!getName().equals(winningBid.getWorkerId())) {
                jobsWaiting.remove(winningBid.getJobId());
                estimatedBids.remove(winningBid.getJobId());
                estimatedWorkCosts.remove(winningBid.getJobId());
            }
    ...
    protected BuiltinStreamConsumer<BidOffer> setupBidOfferConsumer() {
        return bidOffer -> {
            ...
            BidCost bidCost = calculateProcessingTime(job);
            Bid bid = new Bid(job, getName(), bidCost.getWorkCost(), bidCost.getJobName());
            bidsQueue.send(bid);
            jobsWaiting.put(job.jobId, job);
        };
    }

    public WorkCost getWorkCost(String jobId) {
        return estimatedWorkCosts.get(jobId);
    }
    ...
    public void sendWinningBid(Bid bid) {
        try {
            WinningBid winningBid = new WinningBid(bid.getJob().jobId, bid.getJobName(), bid.getWorkerId(), bid.getWorkCost());
            seenWinningBids.add(winningBid);
            winningBids.send(winningBid);

            List<String> workersWorkload = workersWorkloads.getOrDefault(bid.getWorkerId(), new ArrayList<>());
            workersWorkload.add(bid.getJob().jobId);
            workersWorkloads.put(bid.getWorkerId(), workersWorkload);
        } catch (Exception e) {
            throw new RuntimeException(e);
        }
    }
    ...
    private Map<String, Bid> getLosingWorkersForBid(Bid bid) {
        ...
        for (Bid b : bids) {
            if (!b.getWorkerId().equals(bid.getWorkerId())) {
                workerCosts.put(b.getWorkerId(), b);
            }
        }
        return workerCosts;
    }
    ...
    private boolean biddingFinished(String jobId) {
        List<Bid> bids = bidMap.get(jobId);
        return !bids.isEmpty() && bids.size() == getWorkerCount();
    }
...
//                .sum();
    ...
    private void handleHeartBeat(String senderId) {
        if (!lastHeartbeats.containsKey(senderId)) {
            ...
            workersWorkloads.put(senderId, new ArrayList<>());
        }
    ...
    private boolean canTerminate() {
        ...
        return activeJobs.isEmpty() && streamsEmpty;
    }
...
//        return "tcp://" + master + ":" + port + "?wireFormat.maxInactivityDuration=5000000&?wireFormat.maxInactivityDurationInitalDelay=6000000&maxReconnectAttempts=0";
//        return "failover:(tcp://" + master + ":" + port+")?nested.wireFormat.maxInactivityDuration=0";
//        return "failover:(tcp://" + master + ":" + port+")?maxReconnectAttempts=0";
    }
            ...
            for (Stream c : new ArrayList<>(activeStreams)) {
                for (String destinationName : c.getDestinationNames()) {
                    ...
                    try {
                        ...
                        inFlightCount = mbView.getInFlightCount();
...
//            System.out.println("workflow " + getName() + " terminated.");
    ...
    public synchronized void awaitTermination() throws TimeoutException {
        ...
        latestTime = System.currentTimeMillis() - startTime;
    ...
    protected void registerDefaultSerializationTypes(Serializer serializer) {
        ...
        serializer.registerType(EstimationMetrics.class);
        serializer.registerType(ExtendedEstimationMetric.class);
...
//        repository.setWorkerId(job.getJobHash(), workerId);
    ...
    public Future<Bid> publishForBidding(Job job) {
        return bidExecutorService.submit(() -> {
            ...
            if (winningBid.getCost() == Double.MAX_VALUE) {
    ...
    public void ensureWorkersReady() {
        while (workersTooBusy()) {
            sleepMilis(100);
        }
    }

    private boolean workersTooBusy() {
        return workersWorkloads
                .values()
                .stream()
                .mapToInt(List::size)
                .filter(workloadSize -> workloadSize < 4)
                .count() < 1;
    }
    ...
    public boolean workflowReady() {
        List<String> activeWorkers = lastHeartbeats.keySet()
                .stream()
                .filter(workerId -> !workerId.equals(getName()))
                .collect(Collectors.toList());
        return activeWorkers.size() == workerCount;
    }
    ...
    public void saveBidsCsv() {
        String HEADER = "jobId,repositoryName,workerId,cost,jobProcessingTime,bidTime";
        String fileName = "bids.csv";
        try {
            String csv = seenBids.stream()
                    .map(Bid::toCsv)
                    .collect(Collectors.joining("\n"));
    ...
    public void sendEstimationMetrics(EstimationMetrics metrics) {
        try {
            estimationMetricsQueue.send(metrics);
        } catch (Exception e) {
            throw new RuntimeException(e);
        }
    }

    public void sendExtendedEstimationMetrics(ExtendedEstimationMetric metric) {
        try {
            extendedEstimationMetricsQueue.send(metric);
        } catch (Exception e) {
            throw new RuntimeException(e);
        }
    }

    public void sendEstimationMetrics(Job job, double estimatedTime, double actualCost) {
        EstimationMetrics estimationMetrics = new EstimationMetrics(job.jobId, getName(), WorkCost.EMPTY, WorkCost.EMPTY, LocalDateTime.now(), "", 0);
        try {
            estimationMetricsQueue.send(estimationMetrics);
        } catch (Exception e) {
            throw new RuntimeException(e);
        }
    }

    public void sendEstimationMetrics(Job job, WorkCost estimated, WorkCost actual, String repositoryName, long repositorySize) {
        EstimationMetrics estimationMetrics = new EstimationMetrics(job.jobId, getName(), estimated, actual, LocalDateTime.now(), repositoryName, repositorySize);
        try {
            estimationMetricsQueue.send(estimationMetrics);
        } catch (Exception e) {
            throw new RuntimeException(e);
    ...
    public void sendExtendedEstimationMetrics(Job job, WorkCost estimated, WorkCost actual, String repositoryName,
                                              long repositorySize, int fileCount, int lineCount, String commitHash,
                                              double intercept, double slope) {
        String jobName = repositoryName;
        if (!commitHash.isEmpty()) {
            jobName += "__" + commitHash;
        }
        ExtendedEstimationMetric metric = new ExtendedEstimationMetric(job.jobId, getName(), estimated, actual,
                LocalDateTime.now(), jobName, repositorySize, fileCount, lineCount, intercept, slope);
    ...
    public void saveWinningBidsCsv() {
        String HEADER = "jobId,jobName,workerId,winTime,cost";
        String fileName = "winning_bids.csv";
        try {
            String csv = seenWinningBids.stream()
                    .map(WinningBid::toCsv)
                    .collect(Collectors.joining("\n"));
            Files.write(Paths.get(fileName), List.of(HEADER, csv));
            System.out.println("Saved " + fileName);
        } catch (IOException e) {
    ...
    public boolean jobNameExists(String path) {
        return jobNames.stream().anyMatch(jobName -> jobName.contains(path));
    }
}
