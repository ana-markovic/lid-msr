...
public class ResultsSink extends ResultsSinkBase {
    ...
    public void printIndividualTechnologiesCount() {
        Map<String, Integer> techCount = new HashMap<>();
        repoMatrix.forEach((k, v) -> v.forEach(tech -> techCount.put(tech, techCount.getOrDefault(tech, 0) + 1)));
        techCount.forEach((k, v) -> System.out.println(k + " -> " + v));
    }
